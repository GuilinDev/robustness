...
class LIMERobustnessAnalyzer:
    """分析LIME鲁棒性实验结果的类"""
    
    def __init__(self, results_path: str):
        """初始化分析器
        Args:
            results_path: 主结果文件的路径 (lime_robustness_results.json)
        """
    ...
    def plot_metric_heatmaps(self, output_dir: str):
        ...
        for metric in self.metrics:
            ...
            plt.title(f"LIME: {metric_title} by Corruption Type and Severity", fontsize=14, fontweight='bold')
            
            # 添加水印标识
            plt.text(0.98, 0.02, "LIME", transform=plt.gca().transAxes, 
    ...
    def generate_report_table(self, output_path: str, severity_level: int = 3):
        ...
        with open(output_path, 'w') as f:
            f.write('# LIME Robustness Analysis Report\n\n')
            ...
            # 总结发现
            f.write("\n## Summary\n\n")
            f.write("This analysis evaluated the robustness of LIME explanations across 15 different corruption types. ")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Analyze LIME robustness test results')
    parser.add_argument('--results_path', type=str, default='experiments/results/lime_robustness_results.json',
                       help='Path to the LIME robustness results JSON file')
    parser.add_argument('--figures_dir', type=str, default='experiments/results/figures/lime',
                       help='Directory to save the generated figures')
    parser.add_argument('--report_path', type=str, default='experiments/results/lime_analysis_report.md',
