...
class OcclusionSensitivityRobustnessTest:
    """Occlusion Sensitivity鲁棒性测试类"""
    
    def __init__(self, model_type="standard", device: torch.device = None, 
                patch_size: int = 8, stride: int = 4):
        ...
        # Occlusion Sensitivity参数
        self.patch_size = patch_size  # 遮挡块大小
        self.stride = stride          # 滑动步长
        self.baseline_value = 0.5     # 遮挡区域填充值 (灰色)
    ...
    def generate_occlusion_sensitivity(self, image_tensor: torch.Tensor, target_class=None) -> np.ndarray:
        """生成Occlusion Sensitivity解释
        
        使用滑动窗口遮挡图像中的区域，记录每个区域被遮挡后的预测变化
        区域对预测的影响越大，生成的解释中该区域的值越高
        """
        # 获取图像尺寸
        ...
        for h in range(0, height - self.patch_size + 1, self.stride):
            for w in range(0, width - self.patch_size + 1, self.stride):
                # 复制原始图像
                occluded_tensor = image_tensor.clone()
                
                # 在(h, w)位置应用遮挡
                occluded_tensor[0, :, h:h+self.patch_size, w:w+self.patch_size] = self.baseline_value
                
                # 进行预测
                with torch.no_grad():
                    output = self.model(occluded_tensor)
                    score = output[0, target_class].item()
                
                # 计算预测变化并填充敏感度图
                score_diff = original_score - score
                sensitivity_map[h:h+self.patch_size, w:w+self.patch_size] += score_diff
    ...
    def generate_multiple_occlusion_sensitivities(self, image_tensor: torch.Tensor, n_samples: int = 5) -> List[np.ndarray]:
        """生成多次Occlusion Sensitivity解释以计算稳定性
        
        在每次迭代中修改一下patch_size, stride或baseline_value，以获得稍微不同的解释
        """
        explanations = []
        original_patch_size = self.patch_size
        original_stride = self.stride
        original_baseline = self.baseline_value
        
        # 从原始参数稍微偏移
        patch_sizes = [max(4, original_patch_size + i - n_samples//2) for i in range(n_samples)]
        strides = [max(2, original_stride + i - n_samples//2) for i in range(n_samples)]
        baselines = [max(0.1, min(0.9, original_baseline + (i - n_samples//2) * 0.1)) for i in range(n_samples)]
        
        # 生成不同参数组合的解释
        for i in range(n_samples):
            # 修改参数
            self.patch_size = patch_sizes[i]
            self.stride = strides[i]
            self.baseline_value = baselines[i]
            
            # 生成解释
            explanation = self.generate_occlusion_sensitivity(image_tensor)
            explanations.append(explanation)
        
        # 恢复原始参数
        self.patch_size = original_patch_size
        self.stride = original_stride
        self.baseline_value = original_baseline
    ...
    def compute_metrics(self, img_explanation: np.ndarray, corrupted_explanation: np.ndarray, 
        ...
        # 1. 相似度指标 (余弦相似度)
        img_exp_flat = img_explanation.flatten()
        corrupted_exp_flat = corrupted_explanation.flatten()
        
        # 确保数据非空
        if np.sum(img_exp_flat) == 0 or np.sum(corrupted_exp_flat) == 0:
            cosine_sim = 0.0
        else:
            cosine_sim = np.sum(img_exp_flat * corrupted_exp_flat) / (
                np.sqrt(np.sum(img_exp_flat**2)) * np.sqrt(np.sum(corrupted_exp_flat**2)) + 1e-8)
        ...
        if np.sum(img_hist) > 0 and np.sum(corrupted_hist) > 0:
        ...
        if stability_list and len(stability_list) > 1:
            ...
            corr_values = []
            for exp in stability_list:
                if np.sum(exp) > 0 and np.sum(avg_exp) > 0:  # 确保数据非空
                    corr_values.append(np.corrcoef(avg_exp.flatten(), exp.flatten())[0, 1])
            stability = np.mean(corr_values) if corr_values else 0.0
    ...
    def test_robustness(self, image_dir: str, output_file: str, temp_file: str = None, 
        ...
        for idx, image_path in enumerate(image_files):
            ...
            try:
                ...
                # 生成原始图像的解释
                print(f"  生成原始图片的Occlusion Sensitivity解释...")
                original_explanation = self.generate_occlusion_sensitivity(input_tensor, pred_class)
                
                # 生成多个解释用于稳定性计算
                print(f"  生成多个解释用于稳定性计算...")
                original_stability_explanations = self.generate_multiple_occlusion_sensitivities(input_tensor)
                ...
                for c_idx, corruption_type in enumerate(self.corruption_types):
                    ...
                    for severity in range(1, 6):
                        ...
                        # 生成腐蚀后图像的解释
                        corrupted_explanation = self.generate_occlusion_sensitivity(corrupted_tensor, corrupted_pred_class)
                        
                        # 生成多个解释用于稳定性计算
                        corrupted_stability_explanations = self.generate_multiple_occlusion_sensitivities(corrupted_tensor)
...
def main():
    parser = argparse.ArgumentParser(description="测试Occlusion Sensitivity解释的鲁棒性")
    ...
    parser.add_argument('--patch_size', type=int, default=8, help="遮挡区域大小")
    parser.add_argument('--stride', type=int, default=4, help="滑动步长")
    ...
    # 创建测试实例
    tester = OcclusionSensitivityRobustnessTest(
