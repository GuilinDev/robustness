...
mkdir -p experiments/results/deep_lift_robust_figures
...
if [ "$TEST_MODE" = true ]; then
    ...
    TEMP_FILE="experiments/results/deep_lift_robustness_robust_temp.json"
...
if [ $IMAGE_COUNT -gt 0 ]; then
    ...
    # 鲁棒模型通常比标准模型慢约1.5倍
    EST_TIME_PER_IMAGE=$(echo "scale=2; 15 * $STEPS / 5 * 1.5" | bc)
