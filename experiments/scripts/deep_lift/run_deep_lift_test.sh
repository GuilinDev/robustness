#!/bin/bash
# DeepLIFT鲁棒性测试脚本 - 用于在Tiny-ImageNet-200验证集上测试标准ResNet模型
...
STEPS=3            # DeepLIFT稳定性样本数（原始为5，数值越低性能越好）
...
# 确保目录结构存在
mkdir -p experiments/results/deep_lift_viz
mkdir -p experiments/results/deep_lift_standard_figures
...
if [ "$TEST_MODE" = true ]; then
    echo "运行测试模式: 处理20张图像..."
    RESULTS_FILE="experiments/results/deep_lift_robustness_test_results.json"
    TEMP_FILE="experiments/results/deep_lift_test_temp.json"
    VIZ_DIR="experiments/results/deep_lift_viz_test"
    FIGURES_DIR="experiments/results/figures/deep_lift_test"
    REPORT_PATH="experiments/results/deep_lift_test_analysis_report.md"
    
    # 测试模式使用的图像目录限制为20张图像
    TEST_DIR="experiments/data/deep_lift_test_subset"
    mkdir -p "$TEST_DIR"
    ...
    RESULTS_FILE="experiments/results/deep_lift_robustness_standard_results.json"
    TEMP_FILE="experiments/results/deep_lift_robustness_standard_temp.json"
    VIZ_DIR="experiments/results/deep_lift_viz_standard"
    FIGURES_DIR="experiments/results/figures/deep_lift_standard"
    REPORT_PATH="experiments/results/deep_lift_standard_analysis_report.md"
    ...
    SAMPLE_LIST="experiments/data/samples/deep_lift_sample_list.txt"
...
if [ "$OPTIMIZE" = true ]; then
    ...
    echo "已将DeepLIFT稳定性样本数从5减少到 $STEPS 以提高性能"
...
if [ $IMAGE_COUNT -gt 0 ]; then
    ...
    # 估算每张图像处理时间 (基于15秒/图像/样本的粗略估计)
    EST_TIME_PER_IMAGE=$(echo "scale=2; 15 * $STEPS / 5" | bc)
...
if [ ! -f "$RESULTS_FILE" ]; then
    echo "错误: DeepLIFT测试未能生成结果文件: $RESULTS_FILE"
    exit 1
fi
...
# 分析结果
echo "分析结果..."
/workspace/robustness/.venv/bin/python "$SCRIPT_DIR/analyze_deep_lift_robustness_results.py" \
